...
stan_data = list(n=nrow(exposures[subset_samples,]),# 20200119 nrow(exposures),
                 d = ncol(exposures),
...
model_file_name = "../../../ProjectOvarianMultisampleTree/code/files_analysis/modelling_LN/stan_fit_FE_zeros_partialILR.stan"
rstan_options(auto_write = TRUE)
stanc(model_file_name)
...
saveRDS(posterior2, file = paste0("../data/inference/FE_partialILR_posteriors.RDS"))
...
## This is not working as per 20201019
## However, when we simulate data, don't we have to simulate together the zero exposures and the partial ILR subcompositions?
## maybe this would explain why the results from the simulation seem to be all over the place