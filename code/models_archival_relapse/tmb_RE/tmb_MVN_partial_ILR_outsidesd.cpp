// with sd outside of the random effects, with correlations
...
Type objective_function<Type>::operator() ()
{
  ...
  // correlated random effects: full covariance matrix for random effects
    ...
    // vector<Type> vec1(d);
    // vec1.fill(1.0);
    // Sigma.diagonal() = vec1; // diagonal matrix with values from the sd of the subcomposition
  ...
  for(int l=0; l<n; l++){
    ...
    // vec1.fill(1.0);
    Sigma_ones.diagonal() = exp(logsd_subcomposition_l); // diagonal matrix
