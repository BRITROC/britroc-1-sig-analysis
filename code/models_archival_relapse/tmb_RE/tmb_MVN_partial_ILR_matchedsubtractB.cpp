// same as matchedsubtract, but now the paired observations need not hace the same zeros
...
Type objective_function<Type>::operator() ()
{
  ...
  for(int i=0;i<num_individuals;i++){ 
    ...
    // compute how many zero exposures there are, for both observations
    int count_zeros = 0;
    for(int loop_zeros=0; loop_zeros<d; loop_zeros++){
      if( (Y(group_1_idx(i),loop_zeros) == 0) || (Y(group_2_idx(i),loop_zeros) == 0) ){
        count_zeros += 1;
      }
    }

    // if there aren't enough observations (i.e. less than 2, do not run, as there is nothing to ft)

    if(count_zeros < (d-1)){ // at least two entries
