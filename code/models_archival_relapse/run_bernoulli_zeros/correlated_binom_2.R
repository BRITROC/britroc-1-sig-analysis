...
lapply(split_rows(TMB_data$Y, TMB_data$x[,2]), function(i) colSums(i)/nrow(i))
lapply(split_rows(TMB_data$Y, TMB_data$x[,2]), function(i) colSums(i))
lapply(split_rows(TMB_data$Y, TMB_data$x[,2]), function(i) colSums(1-i)) ## only one zero in the second group
...
image(t(TMB_data$Y))
if(read_cached){
  results = readRDS("../out/inference/correlated_binom_2_a8acbdff-a73e-4bdf-8525-f72803583e2d.RDS") ## with a subset of samples
}else{
  TMB::compile("../../models/tmb_RE/tmb_correlated_multinom_2.cpp", "-std=gnu++17")
  dyn.load(dynlib("../../models/tmb_RE/tmb_correlated_multinom_2"))
  bernoulliME
  # results = give_res_report(TMB_data, d)
  results = wrapper_run_TMB_use_nlminb(model = 'bernoulliME', object = TMB_data)
